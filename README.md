...
The Azure defaults move off VM series that Microsoft has announced for retirement or lists as previous-generation. [B-series (V1)](https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/lifecycle/retired-sizes-list#general-purpose-retired-sizes) — the hardware behind the `B_Standard_B1ms` Flexible Server SKU — was announced for retirement on 15 October 2025 and stops running on **15 November 2028**; Microsoft already marks it *capacity limited*, so new allocations can fail well before then. [Dv3 and Dsv3](https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/lifecycle/previous-gen-sizes-list#general-purpose-previous-gen-sizes) — behind `Standard_D2s_v3` and `GP_Standard_D2s_v3` — are not retiring, but they are previous-generation with next-gen replacements available, and Microsoft no longer grows capacity for them, so a node pool scale-out or a server scale on those sizes can fail in a constrained region. Nothing in AWS or GCP changes. Four defaults move:
...
Only the first row is a module default, so it is the only one a plain `ref=<tag>` bump can pull into an existing root: an `aks` module call that never set `default_node_pool_vm_size` gets the new size on its next plan. The other three are literal values in the examples; a root of your own keeps whatever it has until you copy the new values in. Both examples already set `default_node_pool_vm_size` explicitly (`Standard_D4ps_v5` in `simple`, `Standard_D4pds_v6` in `enterprise`), so a root copied from either is not touched by the module default.

**Impact on existing Azure deployments:**

- **Changing the default node pool size rotates the pool.** The `aks` module sets `temporary_name_for_rotation`, so the provider does not replace the cluster: it creates a temporary pool, deletes `default`, recreates it at the new size, and removes the temporary pool. The provider [does not cordon and drain](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/kubernetes_cluster) the system pool during this cycle, so every pod on it is disrupted and rescheduled: the operator and metrics-server, the monitoring stack (Loki, Thanos, Grafana, Alertmanager, kube-state-metrics, Alloy), Ory Kratos and Hydra in `enterprise`, and the AKS system pods, CoreDNS among them. Materialize itself runs on the dedicated Materialize pool and keeps running, but expect a short window of failed DNS lookups and no operator reconciliation while the pool comes back. The rotation needs vCPU quota in the new VM family — Dsv5 for the module default, Dpsv6 for the `simple` example — and briefly for two pools' worth while they overlap. `Dpsv6` also needs a region with Cobalt 100 capacity, which the `Standard_E4pds_v6` Materialize pool already requires, but quota is granted per family, so Epdsv6 quota does not cover Dpsv6. To defer, pin `default_node_pool_vm_size` to your current size in the `aks` module call; the plan then shows no change to the pool.
- **Changing a Flexible Server `sku_name` restarts the server in place.** The provider updates `sku_name` without replacing the resource, and Flexible Server [scales between tiers](https://learn.microsoft.com/en-us/azure/postgresql/scale/concepts-scaling-resources) as well as within one. During the switch no new connections are accepted and uncommitted transactions roll back. For the Materialize metadata database, `GP_Standard_D2s_v3` to `GP_Standard_D2ds_v5` stays within General Purpose — same 2 vCores and 8 GiB, higher IOPS and I/O bandwidth — and qualifies for Microsoft's near-zero-downtime scaling: 10 to 30 seconds, 40 to 60 from the application's side once DNS TTL is counted, provided the delegated subnet has a spare IP address, traffic between addresses in that subnet is allowed, and the region has capacity; without those it falls back to a regular scale of 2 to 10 minutes. `environmentd` loses its metadata connection for that window, so expect a brief outage of the environment and a restart of `environmentd` before it recovers. Schedule the apply for a quiet period.
- **The Ory database move takes the slow path, and it costs more.** Scaling from a 1- or 2-vCore Burstable size is [excluded from near-zero-downtime scaling](https://learn.microsoft.com/en-us/azure/postgresql/scale/concepts-scaling-resources#considerations-and-limitations), so `B_Standard_B1ms` to `GP_Standard_D2ds_v5` is a regular scale: Kratos and Hydra lose their database for 2 to 10 minutes, during which logins, session checks, and token issuance fail. Flexible Server has no Burstable size on non-retiring hardware — the whole Burstable tier is B-series — so the smallest General Purpose size is the only way off it, at roughly ten times the compute price of `B1ms` (see the [Flexible Server pricing page](https://azure.microsoft.com/en-us/pricing/details/postgresql/flexible-server/) for your region). A cross-tier scale can also be refused at apply time with `The requested server edition is incompatible with requested Sku Size` when the target SKU is not offered in the server's region or availability zone; run `az postgres flexible-server list-skus --location <region> -o table` first and confirm `Standard_D2ds_v5` appears under General Purpose. To defer, keep `sku_name = "B_Standard_B1ms"` in your root's `ory_database_config` — and plan the move before 15 November 2028, when B-series stops running.
- **What stays as it was.** `azure/examples/migration` keeps the old sizes on purpose: it adopts a deployment that already exists, and its sizes have to match that deployment so adoption does not rotate a pool or restart a server. The Materialize node pool (`Standard_E4pds_v6`) was already current-generation. The Grafana state database in `azure/modules/monitoring` keeps its `B_Standard_B1ms` default in this release; it sits on the same retiring series and needs the same move before November 2028.
