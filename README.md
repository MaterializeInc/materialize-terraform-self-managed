...
**Also in this release: `aws/modules/monitoring` moves its buckets to the S3 account regional namespace.** This is a separate change from the provider upgrade, and the destructive one. Read it before you apply.

- **Both telemetry buckets are destroyed and recreated, and every Loki log and Thanos metric already in them is lost.** The buckets are now created in your account's regional namespace, which is house policy for new buckets: the name is reserved to your account, so no other account can take it and none can ever take it back. The name changes shape with it, from `<prefix>-mzmon-logs-<random>` to `<prefix>-mzmon-logs-<account>-<region>-an`, and a bucket's name forces a new resource. S3 offers no in-place migration between namespaces — AWS's own guidance is to create the new bucket and copy — so copy anything you need to keep first, or stay on `v11` until you have. Nothing else about the buckets changes: regional endpoints, ARNs, IAM resource patterns, and both backends' configuration are the same as before.
...
- **`aws/modules/monitoring` now floors the `hashicorp/aws` provider at `6.37.0`** (`~> 6.37`), the release that added `bucket_namespace`. `terraform init -upgrade` covers it; the other AWS modules keep `~> 6.0`.
