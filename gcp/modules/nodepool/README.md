...
| <a name="requirement_google-beta"></a> [google-beta](#requirement\_google-beta) | >= 7.22, < 9 |
...
| Name | Version |
| ---- | ------- |
| <a name="provider_google-beta"></a> [google-beta](#provider\_google-beta) | >= 7.22, < 9 |
...
| <a name="input_disk_type"></a> [disk\_type](#input\_disk\_type) | The boot disk type for each node. When null, GKE picks the default for the machine series. Set explicitly when changing an existing pool to a machine series with different disk support (e.g. hyperdisk-balanced for C4/C4A), since the pool otherwise keeps its old disk type. | `string` | `null` | no |
...
| <a name="input_node_locations"></a> [node\_locations](#input\_node\_locations) | List of zones where nodes in this pool will be created. Must be zones within the cluster's region. When null (the default), inherits node\_locations from the cluster. | `list(string)` | `null` | no |
