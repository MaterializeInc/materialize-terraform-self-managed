#!/usr/bin/env python3
"""
Automated Terraform State Migration Tool for GCP
...
class StateMigrator:
    ...
    def _build_rules(self) -> List[MigrationRule]:
        ...
        return [
            ...
            MigrationRule(
                ...
                transform=lambda m: m.group(1),
                description="Move GKE resources from module to root"
            ),

            # =================================================================
            # Database — move from module to root (inline in migration config)
            # =================================================================
            # Skip the time_sleep (not needed for migration, will be recreated)
            MigrationRule(
                pattern=r'^module\.database\.time_sleep\.wait_for_vpc$',
                transform=lambda m: 'time_sleep.wait_for_vpc',
                description="Move database VPC wait to root"
            ),

            # Move other database resources to root
            MigrationRule(
            ...
            # =================================================================
            # The nodepool kubernetes resources (disk setup) will be recreated
            # with different names (old: "${prefix}-mz-swap-disk-setup",
            # new: "disk-setup") but we keep them in state and let terraform
            # handle the diff. The GCP node pool resource stays the same.
            MigrationRule(
            ...
            MigrationRule(
                ...
                description="Rename legacy swap_nodepool[0] to materialize_nodepool"
            ),
    ...
    def detect_module_prefix(self, resources: List[str]) -> Optional[str]:
        ...
        pattern = re.compile(r'^module\.([^.]+)\.module\.(networking|gke|database)')
    ...
    def migrate(self):
        """Run the migration"""
        self.log_section("Automated State Migration (GCP)")
        ...
        try:
            ...
            if len(resources) < 10:
                ...
                self.log(f"", 'ERROR')
                self.log(f"A typical Materialize deployment has 25+ resources.", 'ERROR')
                ...
                self.log(f"  3. State stored elsewhere (GCS bucket, Terraform Cloud, etc.)", 'ERROR')
                self.log(f"", 'ERROR')
            ...
            if self.dry_run:
                ...
                print(f"       - Expect additions: firewall rules for load balancers (new in load_balancers module)")
                print(f"    4. terraform apply")
                print(f"  ")
                print(f"  ℹ️  Some resources may already exist in GCP but weren't in your old state.")
...
def generate_tfvars(old_dir: Path, new_dir: Path):
    ...
    if result.returncode == 0:
        try:
            ...
            # Extract region from GKE cluster
            for resource in state.get('resources', []):
                if resource.get('type') == 'google_container_cluster':
                    instances = resource.get('instances', [])
                    if instances:
                        attrs = instances[0].get('attributes', {})
                        if 'location' in attrs and 'region' not in old_values:
                            old_values['region'] = attrs['location']
                        break

            # Extract prefix from VPC name
            for resource in state.get('resources', []):
                if resource.get('type') == 'google_compute_network':
                    instances = resource.get('instances', [])
                    if instances:
                        name = instances[0].get('attributes', {}).get('name', '')
                        if name and '-network' in name:
                            prefix = name.replace('-network', '')
                            old_values['prefix'] = prefix
                            break

            # Extract project_id from GKE cluster
            for resource in state.get('resources', []):
                if resource.get('type') == 'google_container_cluster':
                    instances = resource.get('instances', [])
                    if instances:
                        project = instances[0].get('attributes', {}).get('project', '')
                        if project:
...
'''
    ...
    if 'project_id' in old_values:
        tfvars_content += f'project_id = "{old_values["project_id"]}"\n'
    else:
        tfvars_content += 'project_id = "your-project-id"  # TODO: Update this\n'
    ...
    if 'prefix' in old_values:
        ...
        tfvars_content += 'prefix = "materialize"  # TODO: Update this\n'
...
# TODO: Set your Materialize instance namespace
...
# TODO: Set your existing mz_system user password (if using Password auth)
...
'''
    ...
    print(f"  - project_id (required)")
    print(f"  - license_key (required)")
    print(f"  - prefix (must match your existing resources)")
    print(f"  - database_password (required)")
    print(f"  - materialize_instance_name (required)")
    print(f"  - materialize_instance_namespace (required)")


def main():
    parser = argparse.ArgumentParser(
        description='Automated Terraform state migration tool for GCP'
    )
    parser.add_argument(
        'old_dir',
        type=Path,
