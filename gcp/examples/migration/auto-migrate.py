...
Migrates resources from the old monolithic GCP module to the new
...
class GCPStateMigrator(BaseStateMigrator):
    provider_name = "GCP"
    module_detection_names = ("networking", "gke", "database")
    expected_resource_count_hint = "25+"
    state_backend_hint = "GCS bucket, Terraform Cloud, etc."
    ...
    def _completion_hints(self) -> List[str]:
        return [
            ...
            "- Expect additions: firewall rules for load balancers (new in load_balancers module)",
        ]

    @staticmethod
    def generate_tfvars(old_dir: Path, new_dir: Path):
        """Generate terraform.tfvars from old GCP configuration"""
