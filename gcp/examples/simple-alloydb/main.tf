...
locals {
  ...
  # Note: AlloyDB creates a 'postgres' database by default
  alloydb_config = {
    user_name = "materialize"
    database  = "postgres" # AlloyDB default database
  }
  ...
  # Connection URL for AlloyDB (same PostgreSQL format)
  metadata_backend_url = format(
    "postgres://%s:%s@%s/%s?sslmode=require&options=-c%%20statement_timeout%%3D%s",
    module.alloydb.users[0].name,
    urlencode(module.alloydb.users[0].password),
    module.alloydb.private_ip,
    local.alloydb_config.database,
...
# Set up AlloyDB cluster for Materialize metadata storage
module "alloydb" {
  ...
  # User configuration (password auto-generated)
  users = [{ name = local.alloydb_config.user_name }]

  # Instance configuration
  cpu_count         = var.alloydb_cpu_count
  availability_type = var.alloydb_availability_type

  # Backup configuration (reduced for experimentation)
  automated_backup_enabled         = true
  continuous_backup_enabled        = true
  continuous_backup_retention_days = 7

  # Disable deletion protection for easier cleanup during experimentation
