...
# AlloyDB specific variables
variable "alloydb_cpu_count" {
  description = "Number of CPUs for AlloyDB primary instance (2, 4, 8, 16, 32, 64, 96, or 128)"
  type        = number
  default     = 2
}

variable "alloydb_availability_type" {
  description = "Availability type for AlloyDB (REGIONAL for HA, ZONAL for single zone)"
  type        = string
  default     = "ZONAL"
}
