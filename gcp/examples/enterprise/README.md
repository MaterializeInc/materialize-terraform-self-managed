...
- `enable_grafana_database`: Give Grafana a dedicated Cloud SQL instance for its own state (defaults to `false`). Without it Grafana keeps users, service accounts and tokens, annotations, dashboard versions, and preferences in SQLite on an `emptyDir` and loses all of it on every restart. Costs a `db-f1-micro`.
- `grafana_host`: Hostname to expose Grafana on through a GCP load balancer (defaults to `null`, which keeps it `ClusterIP` and port-forward-only). Follows `internal_load_balancer` and `ingress_cidr_blocks`; an internal load balancer is allowlisted to the VPC's own subnet ranges. DNS for the name is yours to create. Pair it with `enable_grafana_database` — an exposed Grafana that discards everything its users create is worse than an unreachable one.
