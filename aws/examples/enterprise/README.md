...
- `enable_grafana_database`: Give Grafana a dedicated RDS instance for its own state (defaults to `true`, and only applies when `enable_observability` is on). Without it Grafana keeps users, service accounts and tokens, annotations, dashboard versions, and preferences in SQLite on an `emptyDir` and loses all of it on every restart. Costs a `db.t4g.micro`.
