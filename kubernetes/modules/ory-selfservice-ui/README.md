...
| <a name="input_default_access_token_audience"></a> [default\_access\_token\_audience](#input\_default\_access\_token\_audience) | Audience written onto OAuth2 clients that have none (such as MCP clients registered through dynamic client registration) and granted on their access tokens. Set it to the Materialize MCP resource URL(s). Empty leaves such clients without an audience. | `list(string)` | `[]` | no |
