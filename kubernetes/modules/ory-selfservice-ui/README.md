...
| <a name="input_image_tag"></a> [image\_tag](#input\_image\_tag) | Docker image tag for the selfservice UI. Tracks ory-selfservice releases; pin it (rather than following a floating tag) so upgrades are deliberate, and mirror the same tag for air-gapped installs. | `string` | `"v0.2.2"` | no |
