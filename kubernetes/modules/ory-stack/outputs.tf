...
output "lb_addresses" {
  ...
  value = {
    single_domain = local.single_domain ? try({ ip = kubernetes_service_v1.single_domain_proxy_lb[0].status[0].load_balancer[0].ingress[0].ip, hostname = kubernetes_service_v1.single_domain_proxy_lb[0].status[0].load_balancer[0].ingress[0].hostname }, null) : null
