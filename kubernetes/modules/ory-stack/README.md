...
| <a name="input_oel_registry_host"></a> [oel\_registry\_host](#input\_oel\_registry\_host) | Hostname (no path) of the OEL container registry. Used as the dockerconfigjson auths key. Defaults to the production proxy host. | `string` | `"ory.registry.cloud.materialize.com"` | no |
