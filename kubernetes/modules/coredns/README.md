...
| <a name="input_coredns_image_repository"></a> [coredns\_image\_repository](#input\_coredns\_image\_repository) | Image repository for CoreDNS, without a tag. Override this to pull from a mirror or pull-through cache instead of Docker Hub, for example "myregistry.example.com/coredns/coredns". | `string` | `"coredns/coredns"` | no |
...
| <a name="input_extra_rewrites"></a> [extra\_rewrites](#input\_extra\_rewrites) | Additional CoreDNS `rewrite name` rules injected into the .:53 server block. Each maps an external hostname to an in-cluster target so pods resolve it internally (split-horizon), e.g. reaching a public LoadBalancer FQDN via a ClusterIP to avoid a hairpin back to the cluster's own external LB. `from` is the queried name; `to` is the in-cluster name it resolves to. | <pre>list(object({<br/>    from = string<br/>    to   = string<br/>  }))</pre> | `[]` | no |
