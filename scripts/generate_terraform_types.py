...
def yaml_to_terraform_type(value, depth=2) -> str:
    ...
    if isinstance(value, list):
        if not value:
            return "list(any)"
        elem_type = yaml_to_terraform_type(value[0], depth)
    ...
    if isinstance(value, dict):
        ...
        for k, v in value.items():
            v_type = yaml_to_terraform_type(v, depth + 1)
...
def generate_helm_type_block(helm_yaml: dict) -> str:
    ...
    for key, value in helm_yaml.items():
        tf_type = yaml_to_terraform_type(value, 2)
...
def write_example_override_variables_file(directory: Path, helm_type_block: str, crd_type_block: str) -> bool:
...
def main():
    ...
    for example_dir in [AWS_EXAMPLE_DIR, AZURE_EXAMPLE_DIR, GCP_EXAMPLE_DIR]:
        ...
        print(f"Writing {file_path}...")
        if write_example_override_variables_file(example_dir, helm_type_block, crd_type_block):
            files_updated.append(str(file_path))
